...
graph_type gen_random_boost_lattice(double bond_failure_rate, int dimension, double seed = 0){
    ...
    graph_type graph_boost(dimension * dimension * dimension);
...
int main() {
    ...
    graph_type graphUC2(num_verticesUC2);
    int edgesUC2[25][2] = {
            {0,2}, {1,2}, {2,3}, {2,4},
            {0,6}, {1,8}, {3,9}, {4,11},
            {5,6}, {5,8}, {6,7}, {7,9}, {8,10}, {9,12}, {10,11}, {11,12},
            {6,13}, {8,14}, {9,16}, {11,17},
            {13,15}, {14,15}, {15,16}, {15,17}, {15,1}
    };
    for (int i = 0; i<24; i++){
    ...
    for(int dimension=5;dimension<10;dimension++){
        ...
        for(int i=0;i<sample_size;i++){
            ...
            graph_type graph_boost = gen_random_boost_lattice(0.25, dimension);
            copy_graph(graph_boost, working_graph);
            // set new callback
            vf2_test_callback< graph_type, graph_type > callback(graphUC1, working_graph);
